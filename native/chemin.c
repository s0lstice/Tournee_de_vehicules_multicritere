...
void chemin_base(Donnee *data){
    ...
    if(distance == NULL) fatalerreur(data, "chemin_base : echeque de l'allocation de distance lv 1");

    for(i = 0; i < nb_lieu_total; ++i){
        distance[i] = (int*)malloc(2*sizeof(int));
        if(distance == NULL) fatalerreur(data, "chemin_base : echeque de l'allocation de distance lv 2");
        for(j = 0; j < 2; ++j){
            distance[i][j]= 0;
        }
    }
    ...
    id_sol = next_lieu(data, id_lieu_depart);

    i = nb_lieu_solution(data, 0);
    while(id_sol == -1){
        id_lieu_tmp = id_lieu_solution(data, 0, --i);
        id_sol = next_lieu(data, id_lieu_tmp);
    }

    i = 0;

    while((trouve == 0)&&(tentative != nb_lieu_total)){
        tentative++;
...
/*
        if(id_sol >= nb_lieu_total) id_sol = 0;

        id_lieu_arrive = id_lieu_liste(data, id_sol);
*/
        /*destination future*/
/*
...
/*            if(i >= nb_lieu_total) i = 0;
    ...
    if(++id_sol >= nb_lieu_total) id_sol = 0;
    maj_next_lieu(data, id_lieu_depart, id_sol);

    if(trouve == 0){
        qsort(distance[0], nb_lieu_total -1, sizeof(int*), tris_croissant);
        i = 0;
        while(distance[i][0] == 0) i++;
        id_lieu_arrive = distance[i][1];
    }
*/
    /*on augmente le nombre de fois qu'il a ete visité*/
...
/*    tmp = distance_totale_solution(data, 0) + distance_arc(data, id_lieu_depart, id_lieu_arrive, 0);
    ...
    for(i = 0; i < nb_lieu_total; ++i){
        printf(" distance %d [0] %d [1] %d\n", i, distance[i][0], distance[i][1]);
        free(distance[i]);
    }
    free(distance);
}
